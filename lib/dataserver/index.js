...
var DataServer = function DataServer(host) {
	
	this.dataStore = new DataStore();
	this.peerList  = new PeerList(host);

	var self = this, p,
		bootstrap = this.peerList.getBootStrap(),
		outs = {},
		connections = [];
	this.outs = outs;
	...
	function onMsgPeerList(list, peer){
		console.log('onMsgPeerList: ' + peer.connectedTo, list);
		var pn = self.peerList.getListDiff(list);    	
    	self.peerList.addList(list);
    	if(pn) peer.sendMsg(1, new Buffer(JSON.stringify(self.peerList.list)));
	}
	...
	this.server.listen(opt.get('listenPort'), '127.0.0.1', function() { //'listening' listener
	  console.log('server bound');
	});

	var socksPortNumber = opt.get('socksPortNumber');
	...
	function conectTo(host){
		...
		s.on('connect', function() {
		    ...
		    peer.on('disconnect', function(list, peer){
		    	console.log('outbound connection disconnectd: ' + host);
		    	outs[host] = undefined;
		    	delete outs[host];
		    	connectNext();
		    });
		    ...
		    peer.on('error', function(msg, peer){
		    	console.log('outbound connection error: ' + host);
		    	outs[host] = undefined;
		    	delete outs[host];
		    	connectNext();
		    });
...
DataServer.prototype.insertFile = function insertFile(data){
	...
	var ts = Math.floor(ntp_time.now() / 1000),
		tsBuf = new Buffer(4);
