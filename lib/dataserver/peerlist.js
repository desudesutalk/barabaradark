...
var PeerList = function PeerList(dataDir, host){
	...
	this.list[this.host] = {lastseen: Math.floor(Date.now() / 1000)};
	this.saveList();
};
...
PeerList.prototype.peerSeen = function peerSeen(p){
	if(p in this.list){
		this.list[p].lastseen = Math.floor(Date.now() / 1000);
		this.saveList();
	};
};
...
PeerList.prototype.getBootStrap = function getBootStrap(){
	...
	for (var i = 0; i < bootstrap.length; i++) {
		this.aleadyTried[bootstrap[i].host] = Math.floor(Date.now() / 1000);
	};
