...
Ui.prototype.renderPage = function renderPage() {
	$('#messages').empty().append('<h1 class="page-header"><div class="cont_identi" style="display: inline-block;">' + this.showSub + '</div>&nbsp;' + 
		...
		'<div id="reply_form" style="margin-left: 50px; display: none;"><div class="panel panel-default op">'+
				...
				'<input type="button" value="Post" id="post_edit_send">'+
				...
				'<p>[<a href="javascript:;" class="do_reply" data-parent="'+this.showThread+'">' + (this.showThread === 0 ? 'Create new Thread' : 'Post reply') + '</a>]</p>'+
				'<div id="thread_render"><i class="fa fa-refresh fa-spin"></i> Loading...</div><hr><div id="board_pagination"></div>' +
				(this.showThread === 0 ? '': '<p>[<a href="javascript:;" class="do_reply" data-parent="'+this.showThread+'">Post reply</a>]</p>')
    ...
    $('#reply_form #post_edit_send').on('click', function(){
		...
		this.app.db.addPost(postData, function(){
      		if(this.showThread === 0){
				...
				this.postReceived(hash.toString('hex'));
			}
      	}.bind(this));

      	this.app.ds.insertFile(packed);
      	selectedFile = {};
      	$('#reply_form #reply_text').val('');
      	$('#post_edit_link').click();
      	$('#reply_form').hide();
      	//$('h1').after($('#reply_form').detach());
...
Ui.prototype.renderMsg = function renderMsg(data) {
    ...
    var html = //(data.parent == '0' ? '<hr>': '<br>')+
    	...
    	(data.state == 'sending' || data.state == 'placeholder' ? ' temp_msg' : '') +'" id="msg_' + data.id + '" data-id="' + data.id + '">'+
    	'	<div class="panel-heading">'+
    	...
    	' <i style="color: #999;">(' + dateToStr(data.posted_at) + ') <span class="hidbord_mnu_reply hidbord_clickable" data-id="'+data.id+'" data-parent="'+(data.parent == '0' ? data.id : data.parent)+'">#'+data.id.substr(0, 8)+'</span></i>'+
...
Ui.prototype.bindEvents = function bindEvents(selector) {
	...
	s.find('.panel-body').on('mouseout', function() {
	...
	s.find('span.hidbord_mnu_reply').on('click', function(e){
		...
		this.replyTo = e.target.dataset.parent;
		$('#msg_' + id).parent().after($('#reply_form').detach());

		$('#reply_form div.panel-heading').html('Reply to #'+this.replyTo.substr(0, 8));
    ...
    $('a.do_reply').on('click', function(e){
		e.preventDefault();
		e.stopPropagation();

		this.replyTo = e.target.dataset.parent;
		$(e.target).parent().after($('#reply_form').detach());
		$('#reply_form').show();

		$('#reply_form div.panel-heading').html((this.replyTo == '0' ? '<strong>Create new Thread</strong>': 'Reply to #'+this.replyTo.substr(0, 8)));
...
function handleFileSelect(evt) {
    ...
    selectedFile.isImage = !!files.name.match(/(gif|png|jpe?g)$/i);
    console.dir(files);
    
}

Ui.prototype.postReceived = function postReceived(id) {

	this.app.db.getPost(id).then(function(data){

		if(!data) return false;
		
/*		var notification = new window.Notification("New post",{body: data.message});

		notification.onshow = function () {
		  setTimeout(function() {notification.close();}, 15000);
		};*/

		if(data.parent != this.showThread || this.showThread === 0) return true;

		if(data.id in this.posts){
			$('#thread_render #msg_'+data.id).parent().replaceWith(this.renderMsg(data));
			$('#thread_render #msg_'+ data.id + ' .panel-body').addClass('hidbord_msg_new');
			this.posts[data.id] = data;
			this.renderRefs($('#thread_render #msg_'+ data.id), this.postsRefs);	
			this.bindEvents('#thread_render #msg_' + data.id);
			return true;
		}

		if(this.posts[this.postsIds[this.postsIds.length - 1]].posted_at < data.posted_at){
			$('#thread_render').append(this.renderMsg(data));
			$('#thread_render #msg_'+ data.id + ' .panel-body').addClass('hidbord_msg_new');
			this.postsIds.push(data.id);
			this.posts[data.id] = data;
			this.renderRefs($('#thread_render #msg_'+ data.id), this.postsRefs);	
			this.bindEvents('#thread_render #msg_' + data.id);
			return true;
		...
		var i;
		for (i = 0; i < this.postsIds.length; i++) {
			if(this.posts[this.postsIds[i]].posted_at > data.posted_at) break;
		}

		$('#thread_render #msg_'+this.postsIds[i]).before(this.renderMsg(data));
		$('#thread_render #msg_'+ data.id + ' .panel-body').addClass('hidbord_msg_new');
		
		this.posts[data.id] = data;
		this.postsIds.splice(i, 0, data.id);	
